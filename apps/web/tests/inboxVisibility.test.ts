...
test('「我的消息」当前只展示站内公告，其余通知不露出', () => {
  assert.equal(isVisibleInboxMessage({ kind: 'announce' }), true);
  for (const kind of ['system', 'deliverable', 'user', 'ai_news'] as const) {
    assert.equal(isVisibleInboxMessage({ kind }), false);
  }
});
