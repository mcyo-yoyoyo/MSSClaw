...
function inspectRar(
  ...
  for (const header of fileHeaders) {
    ...
    try {
      entries += 1;
      if (maxEntries !== null && entries > maxEntries) {
        throw limitError(`目录条目超过 ${maxEntries} 个`, 'too_many_entries');
      }
      ...
      if (maxFiles !== null && fileCount > maxFiles) {
        throw limitError(`文件数量超过 ${maxFiles} 个`, 'too_many_files');
      }
...
export async function convertRarToZip(
  ...
  if (maxCompressedBytes !== null && data.byteLength > maxCompressedBytes) {
    throw new PackageZipError(
      'compressed_too_large',
      `RAR 压缩包超过 ${Math.round(maxCompressedBytes / MIB)}MB`,
    );
  }
  ...
  const files = await exclusive(() => extractRar(data, openArchive, options));
  ...
  if (maxCompressedBytes !== null && zipped.byteLength > maxCompressedBytes) {
    throw new PackageZipError(
      'compressed_too_large',
      `RAR 转成 ZIP 后超过 ${Math.round(maxCompressedBytes / MIB)}MB，请精简后再上传`,
    );
  }
  return zipped;
}
