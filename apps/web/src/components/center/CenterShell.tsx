...
export function StatCardGrid({ items, columns = 'auto' }: { items: [string, string | number][]; columns?: 'auto' | 'three' | 'four' }) {
  return (
    <div className={cn('mb-4 grid grid-cols-2 gap-2', columns === 'three' ? 'sm:grid-cols-3' : columns === 'four' ? 'sm:grid-cols-4' : 'sm:grid-cols-[repeat(auto-fit,minmax(9rem,1fr))]')}>
      {items.map(([label, val]) => (
